// Package domain_purchaseorder — pre-bill purchase documents ("Purchase
// Order"). Line items are free-text (no product catalog — invoice-service
// has no Product/Inventory model), scoped the same way Sales Order was: a
// transactional record with a draft lifecycle, not master data.
package domain_purchaseorder
...
// CreateDTO — Number is optional; the service auto-generates one (PO/YYYY/NNNN)
// when blank.
...
// LineCalc / OrderCalc — calcLines' output (app/service/purchase_order.service.go).
...
type IRepository interface {
	Create(dto *CreateDTO, calc *OrderCalc, actorID string) (*model.PurchaseOrder, error)
	Update(companyID, id string, dto *UpdateDTO, calc *OrderCalc, actorID string) (*model.PurchaseOrder, error)
	FindByID(companyID, id string) (*model.PurchaseOrder, error)
...
type IService interface {
	...
	Delete(companyID, id string) error
	Confirm(companyID, actorID, id string) (*model.PurchaseOrder, error)
	BackToDraft(companyID, actorID, id string) (*model.PurchaseOrder, error)
	Cancel(companyID, actorID, id string) (*model.PurchaseOrder, error)
	PreviewNumber(companyID string) (string, error)
}

type ErrNotFound struct{ ID string }

func (e *ErrNotFound) Error() string { return fmt.Sprintf("purchase order %s not found", e.ID) }
...
func (e *ErrNotEditable) Error() string {
	return "a confirmed or cancelled purchase order can't be edited — move it back to draft first"
}
