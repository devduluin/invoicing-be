...
func PurchaseInvoiceRoutes(router fiber.Router, ctrl *controller.PurchaseInvoiceController) {
	...
	g.Get("/summary", middlewares.RequirePermission("invoice-bill-list"), ctrl.Summary)
	...
	g.Put("/:id/template", middlewares.RequirePermission("invoice-bill-update"), ctrl.SetTemplate)
